...
function checkValueAndType(mySet, value) {
  if (mySet.has(value)) {
    ...
    return `Множина не має значення "${value}" типу "${typeof value}`;
  }
  ...
  // Якщо значення не знайдено, повертаємо рядок Множина має значення "${value}" типу "${typeof value}".".
}
...
function setToArray(mySet) {
  const arr = [...mySet];
  const arrSorted = arr.filter((value) => typeof value === "string").sort();
...
function removeDuplicatesInPlace(arr) {
  let noDubles = new Set();
  for (let i = 0; i < arr.length; i++) {
    if (noDubles.has(arr[i])) {
      arr.splice(i, 1);
      i--;
    } else {
      noDubles.add(arr[i]);
